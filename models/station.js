...
class Station {
	...
	async getOnAir(query, wp) {
		...
		return new Response(this.onAir[key].nextUpdate, this.onAir[key].data.data, {
			rpuid: this.data.rpuid,
	...
	async getOnDemand(query, wp, seriesId) {
		...
		if (!this.ondemand) {
			...
			let response = tools.getPaginatedData(
				seriesId ? this.ondemand.seriesId[seriesId] : this.ondemand.pure,
				params,
				query,
				type,
			);
		...
		try {
			...
			if (ondemand.meta.paginated) {
				...
				if (seriesId) {
					if (!this.ondemand.seriesId[seriesId]) {
						this.ondemand.seriesId[seriesId] = {};
					}
					if (!this.ondemand.seriesId[seriesId][ondemand.meta.pageNumber]) {
						this.ondemand.seriesId[seriesId][ondemand.meta.pageNumber] = {};
					}
					this.ondemand.seriesId[seriesId][ondemand.meta.pageNumber][size] = ondemand;
					this.ondemand.seriesId[seriesId][ondemand.meta.pageNumber][size].nextUpdate =
						tools.setCachingTimeout(type, ondemand.meta.cacheExpiresAt);
				} else {
					if (!this.ondemand.pure[ondemand.meta.pageNumber]) {
						this.ondemand.pure[ondemand.meta.pageNumber] = {};
					}
					this.ondemand.pure[ondemand.meta.pageNumber][size] = ondemand;
					this.ondemand.pure[ondemand.meta.pageNumber][size].nextUpdate = tools.setCachingTimeout(
						type,
				...
				if (seriesId) {
					...
					this.ondemand.seriesId[seriesId].nextUpdate = tools.setCachingTimeout(
						type,
						ondemand.meta.cacheExpiresAt,
					);
				} else {
		...
		if (query && query.from) {
			...
			try {
				...
				if (schedule.data[0].meta.paginated) {
					let size = query.size ? query.size : 10;
					if (!this.schedule[schedule.data[0].meta.pageNumber]) {
						this.schedule[schedule.data[0].meta.pageNumber] = {};
					}
					this.schedule[schedule.data[0].meta.pageNumber][size] = schedule;
					this.schedule[schedule.data[0].meta.pageNumber][size].nextUpdate = tools.setCachingTimeout(
						type,
						schedule.data[0].meta.cacheExpiresAt,
					);
					let response = new Response(
						this.schedule[schedule.data[0].meta.pageNumber][size].nextUpdate,
						this.schedule[schedule.data[0].meta.pageNumber][size].data[0].data,
						rqt_params,
					);
	...
	include(keys) {
		...
		return keysList.every((key) => {
			switch (key) {
				case "ids":
					return !!this.data.rpuid;
				case "detail":
					return !!this.data.name && !!this.data.description && !!this.data.country;
				case "bearers":
					return !!this.data.bearerIds && this.data.bearerIds.length > 0;
				case "social":
					return !!this.data.socialIds && this.data.socialIds.length > 0;
				case "streams":
					return !!this.data.liveStreams && this.data.liveStreams.length > 0;
				case "images":
					return !!this.data.multimedia && this.data.multimedia.length > 0;
				default:
					return false;
			}
		});
