	...
	async getOnDemand(query, wp) {
		...
		try {
			...
			if (ondemand.meta.paginated) {
				...
				if (!this.ondemand[ondemand.meta.pageNumber]) {
					this.ondemand[ondemand.meta.pageNumber] = {};
				}
				this.ondemand[ondemand.meta.pageNumber][size] = ondemand;
				this.ondemand[ondemand.meta.pageNumber][size].nextUpdate = tools.setCachingTimeout(
					type,
					ondemand.meta.cacheExpiresAt,
				);
				let response = new Response(
					this.ondemand[ondemand.meta.pageNumber][size].nextUpdate,
					this.ondemand[ondemand.meta.pageNumber][size].data,
					query,
					type,
				);
